...
const getDataDebonuce = debounce(function getData() {
    const entities = {}
    ...
    for (let device in entities) {
        if (device !== 'function') {
            ctx.attributeService.getEntityAttributes({
                id: entities[device].id,
                entityType: 'DEVICE'

            }, 'SERVER_SCOPE', [
                "imgBase64", "factoryNumber", "visibleAttributes", "statesList", 'shiftPositionInRelation']).subscribe(atributes => {
                atributes.forEach(({key, value}) => {
                    try {
                        value = JSON.parse(value)
                    } catch (e) {
                    }
                    entities[device][key] = value
                })
...
function jqueryActions() {
    ...
    $(document).delegate(".entityNameHeader", 'click', function (e) {
        ...
        try {
            ...
            console.log('machine descriptor err', e)
        }
...
function setToolTip() {
    ...
    dateFrom.setUTCHours(2)
    dateFrom.setMinutes(0)
    dateFrom.setSeconds(0)
    dateTo.setSeconds(0)

    if (dateTo.getUTCHours() < 2) {
        dateTo.setDate(dateTo.getDate() - 1)
        dateFrom.setDate(dateTo.getDate())
        dateTo.setUTCHours(19)
        dateTo.setMinutes(0)
    }

    if (dateTo.getUTCHours() > 18) {
        dateTo.setUTCHours(19)
        dateTo.setMinutes(0)
    }
    ...
    <span class="tooltiptext">
        <p>Расчёты за время смены:</p>
        <p>С ${dateFrom.toLocaleString()}</p>
        <p>По ${dateTo.toLocaleString()}</p>
    </span>
    `
    return tooltiptext
}
...
function drawTable(data) {
    ...
    for (let entity in data) {
        const {shiftPositionInRelation: index} = data[entity];
        if (!index)
            return
        entitiesInOrder[index - 1] = entity
    }
    ...
    entitiesInOrder.forEach((entity) => {
            ...
            $('#entitiesTables').append(`
                <div id=${entityNameID} class="entityDiv">
                    ...
                    <div class="box">
                        ...
                        <div id='${entityNameID}_oee' class="column tooltip">
              ...
              <span class="parameter_name">${
                  ...
                  <span>
                    ...
                    'н&nbsp;/&nbsp;д' : value.value == undefined ?
                        'н&nbsp;/&nbsp;д' : value.value.toFixed(1).replace('.', ',') + '%'}
                  </span>
              </span>
              <span id="inWork">
              `
        ...
        $(`#${entityNameID}_entityName`).html(`<h4 title="${name}"><i class="fa fa-angle-right fa-1x"></i>${name}</h4>`)
